...
const HomeFeed = ({ token, currentUser, onViewProfile }) => {
  ...
  // Fetch feed — if empty, fetch popular posts
  const fetchFeed = async () => {
    try {
      const res = await fetch(`${API}/api/posts/feed`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (Array.isArray(data) && data.length > 0) {
        setPosts(data);
        setIsPopular(false);
      } else {
        // No feed posts — fetch popular (all posts ordered by likes)
        const popRes = await fetch(`${API}/api/posts/popular`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchStories = async () => {
    try {
      const res = await fetch(`${API}/api/stories`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
  ...
  const fetchSuggested = async () => {
    try {
      const [allRes, followingRes] = await Promise.all([
        ...
        fetch(`${API}/api/follows/my/following`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleStory = async (e) => {
    ...
    try {
      await fetch(`${API}/api/stories`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: formData });
      fetchStories();
    } catch (err) { console.error(err); }
  };
  ...
  const handleLike = async (postId, isLiked) => {
    try {
      await fetch(`${API}/api/likes/${postId}`, {
  ...
  const fetchComments = async (postId) => {
    try {
      const res = await fetch(`${API}/api/comments/${postId}`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
  ...
  const handleComment = async (postId) => {
    ...
    try {
      await fetch(`${API}/api/comments/${postId}`, {
        method: 'POST',
  ...
  const handleFollow = async (userId) => {
    try {
      await fetch(`${API}/api/follows/${userId}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } });
    ...
    <div style={{ display: 'flex', justifyContent: 'center', background: '#000', minHeight: '100%' }}>
      ...
      <div style={{ width: '100%', maxWidth: 470, paddingBottom: 40 }}>
            ...
            <div style={{ position: 'relative', width: '100%', maxWidth: 400, height: '100vh' }}>
              ...
              <div style={{ position: 'absolute', top: 28, left: 16, right: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 14, border: '2px solid #fff', overflow: 'hidden' }}>
                    {storyView.group.profile_picture
                      ? <img src={`${API}${storyView.group.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
              ...
              {currentStory.media_type === 'video'
                ? <video src={`${API}${currentStory.media_url}`} autoPlay style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <img src={`${API}${currentStory.media_url}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              }
                    ...
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', padding: 2, background: '#000' }}>
                      <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden' }}>
                        {myStories[0].media_type === 'image'
                          ? <img src={`${API}${myStories[0].media_url}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                  ...
                  <div style={{ width: '100%', height: '100%', borderRadius: '50%', padding: 2, background: '#000' }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 'bold', position: 'relative', overflow: 'hidden' }}>
                      {currentUser?.profile_picture
                        ? <img src={`${API}${currentUser.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                  ...
                  <div style={{ width: '100%', height: '100%', borderRadius: '50%', padding: 2, background: '#000' }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#3f3f46', fontWeight: 'bold', fontSize: 20 }}>
                      {group.profile_picture
                        ? <img src={`${API}${group.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
        ...
        {isPopular && posts.length > 0 && (
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #27272a', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {posts.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', color: '#52525b', gap: 12 }}>
            ...
            <p style={{ fontSize: 14, textAlign: 'center' }}>No posts yet.<br />Follow people to see their posts!</p>
          </div>
        )}
          ...
          <div key={post.id} style={{ borderBottom: '1px solid #27272a' }}>
              ...
              <button onClick={() => onViewProfile && onViewProfile(post.user_id)}
                ...
                <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 'bold', flexShrink: 0 }}>
                  {post.profile_picture
                    ? <img src={`${API}${post.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
            ...
            <div style={{ position: 'relative', width: '100%', paddingBottom: '100%', background: '#18181b', overflow: 'hidden' }}>
              {post.media_type === 'video'
                ? <video src={`${API}${post.media_url}`} controls style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
                : <img src={`${API}${post.media_url}`} alt="post" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
              }
            </div>
      ...
      <div className="right-sidebar" style={{ width: 300, flexShrink: 0, padding: '24px 0 24px 24px', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>

        {/* Current user */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <div style={{ width: 44, height: 44, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 16, color: '#fff' }}>
            {currentUser?.profile_picture
              ? <img src={`${API}${currentUser.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
              : currentUser?.username?.[0]?.toUpperCase()
            }
          ...
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: 14, fontWeight: 700, margin: 0, color: '#fff' }}>{currentUser?.username}</p>
            <p style={{ fontSize: 12, color: '#71717a', margin: 0 }}>{currentUser?.full_name || currentUser?.email}</p>
          </div>
        </div>
        ...
        {suggested.length > 0 && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#71717a', margin: 0 }}>Suggested for you</p>
            </div>
            {suggested.map(u => (
              <div key={u.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <button onClick={() => onViewProfile && onViewProfile(u.id)}
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, color: '#fff' }}>
                    {u.profile_picture
                      ? <img src={`${API}${u.profile_picture}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
      ...
      <style>{`
        ...
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #000; } ::-webkit-scrollbar-thumb { background: #27272a; border-radius: 4px; }
      `}</style>
    </div>
  );
};
